...
class ECGToBinaryConverter(QMainWindow):
    ...
    def create_control_panel(self):
        ...
        # Row 0: Record selection, convert button, and info panel toggle
        ...
        self.control_layout.addWidget(self.info_toggle_btn, 0, 4)
    ...
    def update_plot_layout(self):
        ...
        for i, plot in enumerate(self.plot_widgets):
            if self.show_channel[i]:
                ...
                # Update x-axis label for the last visible plot
                if visible_idx == visible_count - 1:
    ...
    def convert_to_binary(self):
        ...
        try:
            ...
            conversion_info += f"Samples converted: {len(scaled_signal):,}\n"
            conversion_info += f"File size: {file_size:,} bytes\n"
            conversion_info += f"Expected size: {expected_size:,} bytes\n"
            conversion_info += f"Scaling: [{min_val:.3f}, {max_val:.3f}] -> [0, 4095]\n"
